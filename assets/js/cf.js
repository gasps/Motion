...
function checkVer() {
    ...
    const ids = ['aboutPageVersion', 'downbarVersion']
    ids.forEach(e => {
        document.getElementById(e).innerHTML = `${currentVersion}`
    })
    fetch("https://motion.gasps.top/VERSION.txt", {
            method: 'GET'
        })
...
// check trafficer version
function checkTMCVer() {
    const versionLabel = document.getElementById('tmcversion');
    ...
    fetch("https://raw.githubusercontent.com/RattlesHyper/TrafficerMC/main/VERSION", {
      ...
      .then(result => {
        if (result.replaceAll("\n", "").replaceAll(" ", "") !== currentVersion) {
          versionLabel.style.display = versionLabel.style.display.replace("none", "");
          createPopup("New version found, please update", "red");
        }
      });
  
    versionLabel.innerHTML = `${currentVersion}`;
  }
